...
$(document).on("click", "#down", function () {
	...
	function (e) {
		...
		margin: 20,
		nav: !0,
		navText: ['<img src="images/packages-left-btn.png" class="img-fluid">', '<img src="images/packages-right-btn.png" class="img-fluid">'],
		dots: !0,
		responsive: {
			0: {
				items: 1
...
$(document).ready(function () {
    ...
    var defaultOptions = {
      ...
      customToggle: $toggle,
      levelSpacing: 40,
    //   navTitle: '<?php echo WEBSITE_NAME ?>',
      levelTitles: true,
...
$('.open_popup').click(function(){
	...
	var priceText 	= $(this).parents('.boxpackages, .upper-combo').find('.pkg_price').text();
  var titleText 	= $(this).parents('.boxpackages, .upper-combo').find('.pkg_title').text();
  var pkgDetails  = $(this).parents('.boxpackages').find('.pricing-list ul').html();

  var cat = $('.nav-link.active').text();
  console.log("Test Cat:" + cat);
  // alert(priceText + titleText)
  $('#popup_form input[name="price"]').val(priceText);
  $('#popup_form input[name="title"]').val('<ul>'+pkgDetails+'</ul>');
  $('#popup_form input[name="pkgtitle"]').val(titleText);
   // $('#popup_form input[name="pkgctgry"]').val(pkgctgry[1]);
